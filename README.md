...
- [ ] Ensure all projects are compatible with Visual Studio 2026.
- [ ] Upgrade .NET Framework 4.x projects to 4.8. This is configured in at least four places for most projects:
...
- [ ] Update NuGet package versions where possible and/or safe.
  ...
  - [ ] Generally, upgrade to the last point release for "core" packages such as EF, SqlClient, Diagnostics, "Extensions", etc.
...
- [ ] If required, disable "Unobtrusive JavaScript" control validation.
- [ ] Remove or disable any hard-coded uses of corporate HTTP web proxies.
...
- [ ] Update all URL references to use HTTPS where possible
  - [ ] Headers, footers, redirect targets, WCF/ASMX endpoint, JavaScript CDN URLs, etc...
  - [ ] Ideally site-relative, e.g.: ("~/foo/script.js")
  - [ ] DO NOT change "http:" in XML schemas or SAML configuration, or anywhere else where it is an "identifier" or "namespace"!
...
- [ ] Remove any hard-coded system paths ("D:\logs", "C:\temp", etc...)
  ...
  - [ ] Failing that, add a config parameter with sane defaults.
...
- [ ] Assume that the platform will be UTC and en-US.
  ...
  - [ ] If containerising, prefer Windows Server 2022 or 2025 container images, which support per-container time zones.
...
- [ ] If any files such as JavaScript or JSON client config varies per-environment, generate them dynamically in ASPX if possible.
...
- [ ] Consider using Azure Key Vault, Azure App Configuration, or a similar "external" configuration source.
...
- [ ] Builds must be successful on a "blank", isolated machine with only Visual Studio on it. (I.e.: an Azure DevOps pipeline agent.)
...
- [ ] Create build pipelines. Make sure to use the new "2.0" pipelines.
  ...
  - [ ] Use "pipeline artifacts" for build outputs files. Avoid zipping outputs (in most cases), as that can interfere with artifact storage deduplication.
...
- [ ] Add the "Source Indexing" task to the pipeline, which then enables Application Insights debug snapshots by automatically download the matching code files.
- [ ] Publish PDB files even in release builds to the PaaS platform. Check the "zip" artefact files to make sure they're there. These are used by both App Service and Application Insights for diagnostics such as crash dump analysis and memory leak analysis.
...
- [ ] Add a "health monitor" endpoint such as "/healthz". This is used by App Service and other load balancers to identify healthy instances.
  ...
  - [ ] Exclude the health monitor path from authentication.
...
- [ ] Integrate the latest [Azure Monitor Open Telemetry Distro](https://learn.microsoft.com/en-us/azure/azure-monitor/app/opentelemetry-configuration?tabs=net) to provide instrumentation.
  - [ ] Convert legacy loging frameworks to use `ILogger` where possible.
  - [ ] Add additional "spans" via `ActivitySource` where applicable, especially for external services that don't auto-instrument such as `SmtpClient`.
